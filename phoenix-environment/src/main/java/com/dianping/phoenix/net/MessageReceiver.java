...
class MessageReceiver {
	...
	public void startServer() {
		...
		if (m_maxThreads > 0) {
			ExecutorService bossExecutor = Threads.forPool().getFixedThreadPool(m_threadNamePrefix + "-Boss" + address,
			      m_maxThreads);
			ExecutorService workerExecutor = Threads.forPool().getFixedThreadPool(m_threadNamePrefix + "-Worker",
			      m_maxThreads);

			factory = new NioServerSocketChannelFactory(bossExecutor, workerExecutor);
		} else {
			ExecutorService bossExecutor = Threads.forPool().getCachedThreadPool(m_threadNamePrefix + "-Boss-" + address);
