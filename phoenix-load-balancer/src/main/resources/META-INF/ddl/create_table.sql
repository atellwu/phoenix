...
CREATE TABLE `deployment_task` (
  ...
  `strategy` varchar(32) DEFAULT NULL COMMENT '部署策略, 可选值: one-by-one, two-by-two, three-by-three',
  `error_policy` int(1) DEFAULT NULL COMMENT '出错处理策略, 可选值: 1 - abort on error, 2 - fall through',
  `auto_continue` int(1) DEFAULT NULL COMMENT '是否自动继续下一个Rollout，可选值: 1 - auto continue, 2 - mannul continue',
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='软负载部署-task表';
