...
public class Handler implements PageHandler<Context> {
	...
	private void showStatus(Context ctx, Model model, Payload payload) {
		String[] idsStr = payload.getIds().split(",");
		...
		for (String idStr : idsStr) {
			...
			if (deployModel != null) {
				try {
					Map<Integer, Map<String, Integer>> progressMap = payload.getProgressMap();
					StatusModelVisitor visitor = new StatusModelVisitor(progressMap);
					deployModel.accept(visitor);
					deployModels.add(visitor.getModel());
				} catch (Exception e) {
					// TODO: handle it;
				}
			}
		}
		Collections.sort(deployModels, new Comparator<DeployModel>() {
			@Override
			public int compare(DeployModel o1, DeployModel o2) {
				return o1.getDomain().compareTo(o2.getDomain());
			}
		});
		model.setDeploys(deployModels);
